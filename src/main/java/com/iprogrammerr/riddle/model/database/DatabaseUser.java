...
public class DatabaseUser implements User {
	...
	String query = queryTemplate.update(new DatabaseTable("user", columns), "id = ?", id);
	...
	String query = queryTemplate.update(new DatabaseTable("user").put(column.key(), column.value()), "id = ?", id);
	session.update(query);
	getAll();
    }
